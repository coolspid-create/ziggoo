...
쿠팡은 로켓직구, G마켓은 해외직구 조건이 적용된 검색 URL로 스캔합니다.
...
`Lens 후보 수집`에는 Google Lens 결과에서 복사한 상품 링크나 검색 결과 텍스트를 붙여넣을 수 있습니다. 이 기능은 붙여넣은 내용 안에서 11번가, 쿠팡, G마켓 링크를 우선 분류하고 기타 쇼핑몰 링크는 참고 후보로 보여줍니다.

`Vision API 보조 분석` 버튼을 누르면 Google Cloud Vision Web Detection을 사용해 결과에서 11번가, 쿠팡, G마켓 URL만 추려 동일 이미지 여부, 상품 URL 패턴, 모델명/제품명/브랜드 일치 정도를 점수화합니다. Lens 검색 후 플랫폼 존재 여부와 모델명 교차 검증을 자동 판정하는 흐름은 다음 단계로 분리합니다.
